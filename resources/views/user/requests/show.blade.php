...
<div class="container mx-auto px-4 py-8">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('user.requests.index') }}" class="text-blue-600 hover:text-blue-800">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
                <div class="flex items-center justify-between mb-6">
                    <h1 class="text-2xl font-bold text-gray-800">Request #{{ $request->id }}</h1>
            ...
            <div class="bg-red-50 border-l-4 border-red-500 rounded-lg p-6 mb-6">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <h3 class="text-lg font-bold text-red-800 mb-2">Request Rejected</h3>
                        <p class="text-red-700 mb-4">{{ $request->admin_comment }}</p>
        ...
        <div class="lg:col-span-1">
            <!-- Request Timeline -->
            <div class="bg-white rounded-lg shadow-lg p-6">
                <h2 class="text-xl font-bold text-gray-800 mb-4">Request Timeline</h2>
                <div class="space-y-4">
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">{{ $request->created_at->format('M d, Y h:i A') }}</p>
                        </div>
                    </div>

                    @if($request->reviewed_at)
                    <div class="flex items-start">
                        <div class="bg-{{ $request->status === 'approved' ? 'green' : 'red' }}-100 p-2 rounded-full mr-3">
                            <i class="fas fa-{{ $request->status === 'approved' ? 'check' : 'times' }}-circle text-{{ $request->status === 'approved' ? 'green' : 'red' }}-600"></i>
                        </div>
                        <div>
                            <p class="font-semibold text-gray-800">{{ ucfirst($request->status) }}</p>
                            <p class="text-sm text-gray-600">{{ $request->reviewed_at->format('M d, Y h:i A') }}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">{{ $request->completed_at->format('M d, Y h:i A') }}</p>
                        </div>
                    </div>
                    @endif
                </div>
...
<!-- Resubmit Modal -->
<div id="resubmitModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <h3 class="text-xl font-bold text-gray-800 mb-4">Resubmit Service Request</h3>
        <form action="{{ route('user.requests.resubmit', $request->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2">Updated Description</label>
            ...
            <div class="mb-4">
                ...
                <textarea name="notes" 
                          rows="4" 
                          class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:border-blue-500"
                          placeholder="Add any additional information based on admin feedback...">{{ $request->notes }}</textarea>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2">Upload Additional Documents (Optional)</label>
                <input type="file" 
                       name="documents[]" 
                       multiple 
                       accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
                       class="w-full border rounded-lg px-4 py-2">
                <p class="text-sm text-gray-600 mt-2">Supported: PDF, DOC, DOCX, JPG, PNG (Max 10MB each)</p>
            </div>
            <div class="flex space-x-3">
                ...
                <button type="submit" 
                        class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded-lg transition">
                    <i class="fas fa-redo mr-2"></i>Resubmit Request
                </button>
            </div>
        </form>
